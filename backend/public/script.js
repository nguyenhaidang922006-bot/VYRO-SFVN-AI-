...
function initGate(){
  if(getCode()){ $("login").classList.add("hidden"); $("app").classList.remove("hidden"); }
}
...
async function loadMarket(){
  try{
    ...
    const t = j.tick || {}, ai = j.ai || {}, candles = j.candles || [];
    setText("status", j.ok ? "LIVE" : "ERROR"); setText("updated", j.updatedAt ? new Date(j.updatedAt).toLocaleTimeString() : "--");
    setText("hcount", candles.length); setText("mode", ai.candleMode || "TICK"); setText("topSymbol", j.symbol || t.symbol || "--");
