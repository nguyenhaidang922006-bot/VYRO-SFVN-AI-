...
const TF_MS = Number(process.env.CANDLE_TF_MS || 60000); // 1 minute
...
async function fetchTick(symbol="CP2CON26"){
  const url = `https://remote.sfvn-invest.com.vn/api/v1/tick/${symbol}/last`;
...
function buildCandle(tick){
  ...
  // Seed candles lần đầu để chart không trống.
  if(candles.length < 5){
    const base = price;
    while(candles.length < 5){
      const i = 5 - candles.length;
      candles.unshift({
...
module.exports = { fetchTick, buildCandle, getCandles };